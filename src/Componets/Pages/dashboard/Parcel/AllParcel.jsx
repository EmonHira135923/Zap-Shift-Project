...
const AllParcel = () => {
  ...
  const { data, isLoading, error } = useParcels(user?.email, search, page);
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-10 pb-20 bg-[#F8FAFC] min-h-screen">
      ...
      <div className="relative min-h-[400px]">
        {authLoading || isLoading ? (
          <AllParcelsPageSkeleton />
        ) : error ? (
          <div className="py-32 text-center bg-white rounded-[3rem] border border-dashed border-red-100 shadow-sm">
            <div className="inline-flex p-8 bg-red-50 rounded-full mb-6">
              <FiPackage size={48} className="text-red-200" />
            </div>
          ...
          <div className="py-32 text-center bg-white rounded-[3rem] border border-dashed border-gray-200 shadow-sm">
            ...
            <h2 className="text-2xl font-bold text-[#002B36]">
              No Parcels Found
            </h2>
            <p className="text-gray-400 mt-2 max-w-xs mx-auto">
              Your delivery list is empty. Start a new shipment to track it
              here!
            </p>
          </div>
        )}
